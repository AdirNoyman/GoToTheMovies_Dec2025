...
// Factory function to create a new MovieRepository instance
...
func (r *MovieRepository) getAllMovies(query string) ([]models.Movie, error) {
	rows, err := r.db.Query(query, defaultLimit)
	if err != nil {
		r.logger.Error("Failed to query movies", err)
		return nil, err
	}
	// Closes the rows Cursor after function completes
	defer rows.Close()

	// We are using a slice to hold multiple movies and a Cursor to iterate over the rows so each row (m) is mapped (scan and map) to a movie struct
...
func (r *MovieRepository) GetTopMoviesFromRepo() ([]models.Movie, error) {
	// Fetch movies
	query := `
...
func (r *MovieRepository) GetRandomMoviesFromRepo() ([]models.Movie, error) {
	// Fetch movies
	query := `
		SELECT id, tmdb_id, title, tagline, release_year, overview, score, 
		       popularity, language, poster_url, trailer_url
...
func (r *MovieRepository) GetMovieByIDFromRepo(id int) (models.Movie, error) {
	// Fetch movie
	query := `
		SELECT id, tmdb_id, title, tagline, release_year, overview, score, 
		       popularity, language, poster_url, trailer_url
...
func (r *MovieRepository) SearchMoviesByNameFromRepo(name string, order string, genre *int) ([]models.Movie, error) {
...
func (r *MovieRepository) GetAllGenresFromRepo() ([]models.Genre, error) {
